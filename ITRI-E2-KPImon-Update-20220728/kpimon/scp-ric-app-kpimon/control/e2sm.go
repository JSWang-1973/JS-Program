...
//	if plmnID.Size != 3 || nrCellID.Size != 5 { // wrong data checking, removed by sww, ITRI
	if nRCGI.PlmnID.Size != 3 || nRCGI.NRCellID.Size != 5 { // updated by sww, ITRI
	...
	nodeId := fmt.Sprintf("%x%x%x%x%x%x%x%x%x", int(former[0]), int(former[1]), int(former[2]), int(latter[0]), int(latter[1]), int(latter[2]), int(latter[3]), int(latter[4]), int(latter[5]) )
	CellID = CellID + nodeId
...
//	CellID = CellID + strconv.Itoa(int(former[0])) + strconv.Itoa(int(former[1])) + strconv.Itoa(int(former[2])) + strconv.Itoa(int(latter[0])) + strconv.Itoa(int(latter[1])) + strconv.Itoa(int(latter[2])) + strconv.Itoa(int(latter[3])) + strconv.Itoa(int(latter[4])) + strconv.Itoa(int(latter[5]))
	...
	fmt.Printf("\nParseNRCGI plmnID.Size/nrCellID.Size=%d %d CellID=%s\n", plmnID.Size, nrCellID.Size, CellID)
	return
}
...
// added by sww, ITRI for CellRF parsing
func (c *E2sm) ParseCellRF(buffer []byte, size int) (cellRF *CellRFType, err error) {
	if size < 12 {
		return nil, errors.New("Invalid input: illegal length of cellRF")
	}
	var temp uint8
	var byteBuffer *bytes.Buffer
	var index int
	...
	for index := 0; index < 4; index++ {
		byteBuffer = bytes.NewBuffer(buffer[index : index+1])
		binary.Read(byteBuffer, binary.BigEndian, &temp)
		rsrp = rsrp*256 + int32(temp)
	}

	for index = 4; index < 8; index++ {
		byteBuffer = bytes.NewBuffer(buffer[index : index+1])
		binary.Read(byteBuffer, binary.BigEndian, &temp)
		rsrq = rsrq*256 + int32(temp)
	}

	for index = 8; index < 12; index++ {
		byteBuffer = bytes.NewBuffer(buffer[index : index+1])
		binary.Read(byteBuffer, binary.BigEndian, &temp)
		rsSinr = rsSinr*256 + int32(temp)
	}

	cellRF = &CellRFType{RSRP: rsrp, RSRQ: rsrq, RSSINR: rsSinr}
	return
}

// added by sww, ITRI for CellRF parsing
func (c *E2sm) ParseBWPData(buffer []byte, size int) (bwpData *BWPType) {
	if size < 20 {
		return nil
	}
	var temp uint8
	...
	for index := 12; index < 16; index++ {
		byteBuffer = bytes.NewBuffer(buffer[index : index+1])
		binary.Read(byteBuffer, binary.BigEndian, &temp)
		bwpId = bwpId*256 + int32(temp)
	}

	for index = 16; index < 20; index++ {
		byteBuffer = bytes.NewBuffer(buffer[index : index+1])
		binary.Read(byteBuffer, binary.BigEndian, &temp)
		bandwidth = bandwidth*256 + int32(temp)
	}

	bwpData = &BWPType{BWPID: uint16(bwpId), LocationAndBandwidth: uint16(bandwidth)}
	return
}

// added by sww, ITRI for CellRF parsing
func (c *E2sm) ParseCID(buffer []byte, size int) (CellID string, err error) {
//	fmt.Printf("SWW ParseCID. buffer=%v\n", buffer)
	if size < 8 {
		return "", errors.New("Invalid input: illegal length of CID")
	}
        bitsUnused := 4;
	CellID, _ = c.ParsePLMNIdentity(buffer, 3)

	var former []uint8 = make([]uint8, 3)
	var latter []uint8 = make([]uint8, 6)

	former[0] = buffer[3] >> 4
	former[1] = buffer[3] & 0xf
	former[2] = buffer[4] >> 4
	latter[0] = buffer[4] & 0xf
	latter[1] = buffer[5] >> 4
	latter[2] = buffer[5] & 0xf
	latter[3] = buffer[6] >> 4
	latter[4] = buffer[6] & 0xf

//	fmt.Printf("SWW ParseCID. buffer=%v\n", buffer)
	latter[5] = buffer[7] >> uint(bitsUnused)
...
//	CellID = CellID + strconv.Itoa(int(former[0])) + strconv.Itoa(int(former[1])) + strconv.Itoa(int(former[2])) + strconv.Itoa(int(latter[0])) + strconv.Itoa(int(latter[1])) + strconv.Itoa(int(latter[2])) + strconv.Itoa(int(latter[3])) + strconv.Itoa(int(latter[4])) + strconv.Itoa(int(latter[5]))
	...
	fmt.Printf("ParseCID: CellID=%s\n", CellID)
	return
}

// added by sww, ITRI for CellRF parsing
func (c *E2sm) ParseNeighborCellRF(buffer []byte, size int) (neighborCellRF *NeighborCellRFType, err error) {
	if size < 20 {
		return nil, errors.New("Invalid input: illegal length of neighborCellRF")
	}
	var byteBuffer *bytes.Buffer
	byteBuffer = bytes.NewBuffer(buffer[0 : 8])
	cellID, _ := c.ParseCID(byteBuffer.Bytes(), 8)
	byteBuffer = bytes.NewBuffer(buffer[8 : 20])
	cellRF, _ := c.ParseCellRF(byteBuffer.Bytes(), 12)

	neighborCellRF = &NeighborCellRFType{CellID: cellID, CellRF: *cellRF}
	return
}

// added by sww, ITRI for CellRF parsing
func (c *E2sm) ParseNeighborCellRFList(buffer []byte, size int) (neighborCellRFList []NeighborCellRFType, err error) {
	rfLen := 20; // size of Int * 3 + size of PlmnId + size of cid
	if size < 20 {
		return nil, errors.New("Invalid input: illegal length of neighborCellRF")
	}
        count := size / rfLen;
	...
	fmt.Printf("ParseNeighborCellRFList size=%d count=%d\n", size, count)
	var byteBuffer *bytes.Buffer
	var cellRF *NeighborCellRFType

	for index := 0; index < count; index++ {
		byteBuffer = bytes.NewBuffer(buffer[index*rfLen : (index+1)*rfLen])
		cellRF, _ = c.ParseNeighborCellRF(byteBuffer.Bytes(), rfLen)
		neighborCellRFList[index] = *cellRF
	}

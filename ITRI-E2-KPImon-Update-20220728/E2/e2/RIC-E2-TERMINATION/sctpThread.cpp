...
{
    mdclog_severity_t level = MDCLOG_DEBUG; // modified by sww, itri
    ...
    printf("update_mdc_log_level_severity: level=%d \n", level); // modified by sww, itri
...
{
    printf("dynamic_log_level_change... \n"); // modified by sww, itri
...
int sendSctpMsg(ConnectedCU_t *peerInfo, ReportingMessages_t &message, Sctp_Map_t *m) {
    ...
    while (true) {
        ...
        printf("SWW: %s: send report.buffer of size buildJsonMessage...\n", __func__);
...
void getRequestMetaData(ReportingMessages_t &message, RmrMessagesBuffer_t &rmrMessageBuffer) {
    ...
    // added by sww, if need to print pdu
/*
    if (mdclog_level_get() >= MDCLOG_DEBUG) {
 ...
 *  in edge-triggered mode and won't get a notification again for the same data. */
    ...
    while (true) {
        ...
        printf("message.message.asnLength=%d\n", message.message.asnLength); // added by sww, itri
        if (loglevel >= MDCLOG_DEBUG) {
...
//                pdu = nullptr;
        ...
        numOfMessages++;
        if (pdu != nullptr) {
            mdclog_write(MDCLOG_DEBUG, "SWW: %s: ASN_STRUCT_FREE...", __func__ );
//            ASN_STRUCT_RESET(asn_DEF_E2AP_PDU, pdu);
            ASN_STRUCT_FREE(asn_DEF_E2AP_PDU, pdu); // disabled for TEST
            pdu = nullptr;
        }
#ifdef UNIT_TEST
    done = 1;
...
//    }
    ...
    printf("SWW: %s: gotSetup... buildJsonMessage...\n", __func__);
    buildJsonMessage(message);
    ...
    for (auto i = 0; i < pdu->choice.initiatingMessage->value.choice.E2setupRequest.protocolIEs.list.count; i++) {
        ...
        if (ie->id == ProtocolIE_ID_id_GlobalE2node_ID) {
            ...
            if (ie->value.present == E2setupRequestIEs__value_PR_GlobalE2node_ID) {
                ...
                mdclog_write(MDCLOG_INFO, "collectSetupRequestData buildRanName: %s", message.peerInfo->enodbName); // added by sww
            }
    ...
    switch (rmrMessageBuffer.rcvMessage->mtype) {
        case RIC_E2_SETUP_RESP : {
            if (loglevel >= MDCLOG_DEBUG) {
                ...
                mdclog_write(MDCLOG_DEBUG, "send E2 Setup Response to %s", message.message.enodbName); // added by sww, itri
            }
        ...
        case RIC_E2_SETUP_FAILURE : {
            if (loglevel >= MDCLOG_DEBUG) {
                ...
                mdclog_write(MDCLOG_DEBUG, "send E2 Setup Failure to %s", message.message.enodbName); // added by sww, itri
            }
            if (PER_FromXML(message, rmrMessageBuffer) != 0) {
                break;
            }
        ...
        case RIC_SUB_REQ: {
            if (loglevel >= MDCLOG_DEBUG) {
                ...
                mdclog_write(MDCLOG_DEBUG, "send RIC Subscription Request to %s", message.message.enodbName); // added by sww, itri
            }
#if !(defined(UNIT_TEST) || defined(MODULE_TEST))
...
//          printf("SWW: rmr_rts_msg ep_name: %s\n", (char *) ((uta_mhdr_t_ *) rmrMessageBuffer.rcvMessage->header)->src);
...
void buildJsonMessage(ReportingMessages_t &message) {
    ...
    jsonTrace = true;
#endif
    jsonTrace = false; // disabled by sww, ITRI
    if (jsonTrace) {
